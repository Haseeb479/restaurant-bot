...
class WhatsAppAiBotService
{
    ...
    public function handle(Restaurant $restaurant, string $customerPhone, string $recipientJid, string $messageText, ?array $locationCoords = null): void
    {
        ...
        // ── Deterministic Rule-Based Menu Ordering Engine ────────────────────
        $flowResult = $this->processMenuFlow($restaurant, $customerPhone, $recipientJid, $text, $sessionKey);
        if ($flowResult !== null) {
            BotEvolutionClient::sendMessage($restaurant, $recipientJid, $flowResult['reply']);
            $history[] = ['role' => 'user', 'content' => $text];
            $history[] = ['role' => 'assistant', 'content' => $flowResult['reply']];
            if (count($history) > 20) {
                $history = array_slice($history, -20);
            }
            Cache::put($sessionKey, $history, now()->addMinutes(self::SESSION_TTL));
...
PROMPT;
    ...
    // ──────────────────────────────────────────────────────────────────────────
    //  Rule-Based Menu Flow Engine
    // ──────────────────────────────────────────────────────────────────────────

    public function processMenuFlow(Restaurant $restaurant, string $customerPhone, string $recipientJid, string $text, string $sessionKey): ?array
    {
        $flowStateKey = "wa_flow_state_{$restaurant->id}_{$customerPhone}";
        $cartKey      = "wa_cart_{$restaurant->id}_{$customerPhone}";
        $orderDataKey = "wa_order_data_{$restaurant->id}_{$customerPhone}";
        $catKey       = "wa_selected_cat_{$restaurant->id}_{$customerPhone}";

        $flowState = Cache::get($flowStateKey, 'IDLE');
        $cart      = Cache::get($cartKey, []);
        $orderData = Cache::get($orderDataKey, [
            'customer_name'    => null,
            'contact_phone'    => null,
            'delivery_address' => null,
            'payment_method'   => 'cash_on_delivery',
        ]);
        $selectedCatId = Cache::get($catKey, null);

        $lower = mb_strtolower(trim($text));

        // 1. Detect if this message pertains to the menu flow:
        $isFlowCommand = (bool) preg_match('/^(?:menu|منو|مینو|categories|items|list|card|start|cart|clear|khali|checkout|cancel|radd|khatam|0|back|wapas)$/iu', $lower);
        $isNumeric = (bool) preg_match('/^\d+(?:\s*[*xX×]\s*\d+)?$/u', $lower);

        if ($flowState === 'IDLE' && ! $isFlowCommand && ! $isNumeric) {
            // Check if it's a greeting
            if (preg_match('/^(?:hi|hello|hey|salam|assalam|aoa)\b/iu', $lower)) {
                return [
                    'reply' => $this->buildCategoriesMenu($restaurant),
                    'orderReady' => false,
                ];
            }
            // Otherwise let it pass to Groq / smart fallback
            return null;
        }

        // 2. Global Commands:
        if (preg_match('/^(?:cancel|radd|khatam|exit)$/iu', $lower)) {
            Cache::forget($cartKey);
            Cache::put($flowStateKey, 'IDLE', now()->addMinutes(self::SESSION_TTL));
            return [
                'reply' => "❌ *Order cancel kar diya gaya hai.*\n\nNai shuruat ke liye *menu* likhein! 😊",
                'orderReady' => false,
            ];
        }

        if (preg_match('/^(?:clear|empty|khali)$/iu', $lower)) {
            Cache::forget($cartKey);
            Cache::put($flowStateKey, 'IDLE', now()->addMinutes(self::SESSION_TTL));
            return [
        ...
        if (preg_match('/^(?:cart|basket|bill)$/iu', $lower)) {
            $deliveryCharge = (float) ($restaurant->delivery_charge ?? 0);
            return [
                'reply' => $this->buildCartSummary($cart, $deliveryCharge),
                'orderReady' => false,
            ];
        }

        if (in_array($lower, ['0', 'back', 'wapas', 'menu', 'categories'], true) || ($lower === 'start' && $flowState === 'IDLE')) {
            Cache::put($flowStateKey, 'IDLE', now()->addMinutes(self::SESSION_TTL));
            return [
                'reply' => $this->buildCategoriesMenu($restaurant),
                'orderReady' => false,
        ...
        if (preg_match('/^(?:checkout|check out|place order|order kar do|confirm order)$/iu', $lower)) {
            if (empty($cart)) {
                return [
                    'reply' => "🛒 *Aapka cart abhi khali hai!*\n\nPehle menu se items select karein:\n\n" . $this->buildCategoriesMenu($restaurant),
                    'orderReady' => false,
                ];
            }
            return $this->startFlowCheckout($restaurant, $customerPhone, $flowStateKey, $orderDataKey, $sessionKey);
        }

        // 3. State Router:
        switch ($flowState) {
            case 'IDLE':
                $categories = $this->getFlowCategories($restaurant);
                $catNum = (int) $lower;
                if ($catNum >= 1 && $catNum <= count($categories)) {
                    $chosen = $categories[$catNum - 1];
                    Cache::put($catKey, $chosen['id'], now()->addMinutes(self::SESSION_TTL));
                    Cache::put($flowStateKey, 'BROWSING_CATEGORY', now()->addMinutes(self::SESSION_TTL));
                    return [
                        'reply' => $this->buildCategoryItemsMenu($restaurant, $chosen['id']),
                        'orderReady' => false,
                    ];
                }
                return [
            ...
            case 'BROWSING_CATEGORY':
                $items = $this->getFlowItemsForCategory($restaurant, $selectedCatId);
                $added = $this->parseFlowItemSelections($text, $items);
                if (! empty($added)) {
                    foreach ($added as $add) {
                        $this->addFlowCartItem($cart, $add['item'], $add['quantity']);
                    }
                    Cache::put($cartKey, $cart, now()->addMinutes(self::SESSION_TTL));

                    $delivery = (float) ($restaurant->delivery_charge ?? 0);
                    $subtotal = 0.0;
                    foreach ($cart as $ci) {
                    ...
                    $addedSummary = implode("\n", array_map(fn ($a) => "• {$a['quantity']}x {$a['item']['name']} (Rs. " . ($a['item']['price'] * $a['quantity']) . ")", $added));

                    return [
                        'reply' => "✅ *Cart mein add ho gaya:*\n{$addedSummary}\n\n" .
                                   "🛒 *Subtotal:* Rs.{$subtotal} | *Total with Delivery:* Rs.{$total}\n\n" .
                ...
                // Check if user typed another category number directly
                $categories = $this->getFlowCategories($restaurant);
                $catNum = (int) $lower;
                if ($catNum >= 1 && $catNum <= count($categories)) {
                    $chosen = $categories[$catNum - 1];
            ...
            case 'CHECKOUT_NAME':
                $name = trim(preg_replace('/[*_`]/u', '', $text));
                if (mb_strlen($name) < 2) {
                    return [
                ...
                Cache::put($orderDataKey, $orderData, now()->addMinutes(self::SESSION_TTL));
                Cache::put($flowStateKey, 'CHECKOUT_PHONE', now()->addMinutes(self::SESSION_TTL));
                return [
            ...
            case 'CHECKOUT_PHONE':
                $phone = $text;
                if (in_array($lower, ['same', 'same number', 'yahi', 'yahi number'], true)) {
                    $phone = $customerPhone;
                } else {
                ...
                $orderData['contact_phone'] = $phone;
                Cache::put($orderDataKey, $orderData, now()->addMinutes(self::SESSION_TTL));

                // Check if delivery address or location pin is already cached
                $cachedAddr = Cache::get("verified_delivery_address_{$sessionKey}");
                if ($cachedAddr || ! empty($orderData['delivery_address'])) {
                    $orderData['delivery_address'] = $orderData['delivery_address'] ?: $cachedAddr;
                    Cache::put($orderDataKey, $orderData, now()->addMinutes(self::SESSION_TTL));
                    Cache::put($flowStateKey, 'ORDER_CONFIRMATION', now()->addMinutes(self::SESSION_TTL));
                    $delivery = (float) ($restaurant->delivery_charge ?? 0);
                    return [
                        'reply' => $this->buildOrderSummaryFromCart($cart, $orderData, $delivery),
                        'orderReady' => false,
                    ];
                }

                Cache::put($flowStateKey, 'CHECKOUT_ADDRESS', now()->addMinutes(self::SESSION_TTL));
                return [
            ...
            case 'CHECKOUT_ADDRESS':
                $addr = trim(preg_replace('/[*_`]/u', '', $text));
                if (mb_strlen($addr) < 3) {
                    return [
                ...
                $orderData['delivery_address'] = $addr;
                Cache::put($orderDataKey, $orderData, now()->addMinutes(self::SESSION_TTL));
                Cache::put($flowStateKey, 'ORDER_CONFIRMATION', now()->addMinutes(self::SESSION_TTL));
                $delivery = (float) ($restaurant->delivery_charge ?? 0);
                return [
            ...
            case 'ORDER_CONFIRMATION':
                if (str_contains($lower, 'jazzcash') || str_contains($lower, 'jazz cash')) {
                    $orderData['payment_method'] = 'jazzcash';
                    Cache::put($orderDataKey, $orderData, now()->addMinutes(self::SESSION_TTL));
                    $delivery = (float) ($restaurant->delivery_charge ?? 0);
                    return [
                        'reply' => "✅ Payment method *JazzCash* select ho gaya hai!\n\n" . $this->buildOrderSummaryFromCart($cart, $orderData, $delivery),
                        'orderReady' => false,
                    ];
                }

                if (str_contains($lower, 'easypaisa') || str_contains($lower, 'easy paisa')) {
                ...
                if ($lower === 'cod' || str_contains($lower, 'cash on delivery') || $lower === 'cash') {
                ...
                $isAffirmative = (bool) preg_match('/^(?:yes|haan|ha|confirm|theek hai|thk hai|ok|kr do|kar do|done|jee|ji|bhej do|order confirm|placed)$/iu', trim($lower));

                if ($isAffirmative) {
                    $trackingCode = $this->saveOrderFromCart($restaurant, $customerPhone, $cart, $orderData, $sessionKey);
                    if ($trackingCode) {
                        $savedOrder = Order::where('tracking_code', $trackingCode)->first();
                        $totalStr = $savedOrder ? number_format((float) $savedOrder->total, 0) : '0';
                        $payMethod = ($savedOrder->payment_method ?? 'cash_on_delivery') === 'jazzcash'
                            ? 'JazzCash 📱'
                            : (($savedOrder->payment_method ?? '') === 'easypaisa' ? 'EasyPaisa 📱' : 'Cash on Delivery (COD) 💵');

                        $trackUrl = url('/track/' . $trackingCode);
                        $reply = "🎉 *Shukriya! Aapka order confirm ho chuka hai!* ✅\n\n" .
                                 "📋 *Tracking Code:* *{$trackingCode}*\n" .
                                 "💵 *Total Payable:* *Rs.{$totalStr}* ({$payMethod})\n" .
                                 "📍 *Deliver to:* {$savedOrder->delivery_address}\n\n" .
                                 "🔗 *Live Order Tracking:* {$trackUrl}\n\n" .
                        ...
                        if ($notifyPhone) {
                            $itemsStr = '';
                            if ($savedOrder && $savedOrder->items()->exists()) {
                                $itemsStr = "\n🍽️ *Items:* " . $savedOrder->items->map(fn ($i) => "{$i->quantity}x {$i->name}")->implode(', ');
                            }
                            $ownerMsg = "🔔 *New Order — {$restaurant->name}!*\n\n" .
                                        "📦 *#{$trackingCode}*\n" .
                                        "📱 *Customer:* {$customerPhone}" .
                                        $itemsStr .
                                        "\n💰 *Total:* Rs. {$totalStr}" .
                        ...
                        // Clear cart and flow state
                        Cache::forget($cartKey);
                        Cache::forget($orderDataKey);
                        Cache::forget($flowStateKey);
                        Cache::forget($sessionKey);
                        ...
                        Cache::put($flowStateKey, 'IDLE', now()->addMinutes(self::SESSION_TTL));
                        return [
                            'reply' => "⚠️ Sorry — something went wrong saving your order, so it has *not* been placed.\n\nPlease send your order again in a moment, or contact us directly.",
                            'orderReady' => false,
                        ];
                    }
                }
    ...
    public function saveOrderFromCart(Restaurant $restaurant, string $customerPhone, array $cart, array $orderData, string $sessionKey): ?string
    {
        if (empty($cart)) {
            return null;
        }
        ...
        $cachedGps = Cache::get("verified_delivery_coords_{$sessionKey}");
        $deliveryLat = $cachedGps ? (float) $cachedGps[0] : (isset($orderData['delivery_lat']) ? (float) $orderData['delivery_lat'] : null);
        $deliveryLng = $cachedGps ? (float) $cachedGps[1] : (isset($orderData['delivery_lng']) ? (float) $orderData['delivery_lng'] : null);
        $deliveryAddress = $orderData['delivery_address'] ?? (Cache::get("verified_delivery_address_{$sessionKey}") ?: 'Collected via WhatsApp');

        $paymentMethod = $orderData['payment_method'] ?? 'cash_on_delivery';
        $customerName = $orderData['customer_name'] ?? null;
        $contactPhone = $orderData['contact_phone'] ?? $customerPhone;
        ...
        try {
            ...
            DB::rollBack();
            Log::error("WhatsApp AI: Error saving order from cart: " . $e->getMessage());
            return null;
        }
    }

    private function startFlowCheckout(Restaurant $restaurant, string $customerPhone, string $flowStateKey, string $orderDataKey, string $sessionKey): array
    {
        $orderData = Cache::get($orderDataKey, []);
        if (empty($orderData['customer_name'])) {
            Cache::put($flowStateKey, 'CHECKOUT_NAME', now()->addMinutes(self::SESSION_TTL));
            return [
        ...
        if (empty($orderData['contact_phone'])) {
            Cache::put($flowStateKey, 'CHECKOUT_PHONE', now()->addMinutes(self::SESSION_TTL));
            return [
                'reply' => "📝 *Order Booking — Step 2/3*\n\nRabtay ke liye *Phone Number* batayein 📱\n_(Ya agar yahi WhatsApp number hai toh *'same'* likhein)_:",
                'orderReady' => false,
        ...
        if ($cachedAddr || ! empty($orderData['delivery_address'])) {
            ...
            Cache::put($flowStateKey, 'ORDER_CONFIRMATION', now()->addMinutes(self::SESSION_TTL));
            $delivery = (float) ($restaurant->delivery_charge ?? 0);
            $cart = Cache::get("wa_cart_{$restaurant->id}_{$customerPhone}", []);
            return [
                'reply' => $this->buildOrderSummaryFromCart($cart, $orderData, $delivery),
                'orderReady' => false,
            ];
    ...
    private function getFlowCategories(Restaurant $restaurant): array
    {
        $categories = [];
        if (method_exists($restaurant, 'deals')) {
            $dealsCount = $restaurant->deals()->where('is_active', true)->count();
            if ($dealsCount > 0) {
                $categories[] = ['id' => 'deals', 'name' => 'Special Deals & Offers 🌟'];
            }
        }

        $cats = $restaurant->categories()->where('is_active', true)->orderBy('sort_order')->get();
        foreach ($cats as $cat) {
            $categories[] = ['id' => (string) $cat->id, 'name' => $cat->name];
        }

        if (empty($categories)) {
            $categories[] = ['id' => 'all', 'name' => 'Main Menu 🍔'];
        }

        return $categories;
    }

    private function getFlowItemsForCategory(Restaurant $restaurant, $categoryId): array
    {
        $items = [];
        if ($categoryId === 'deals') {
            $deals = $restaurant->deals()->where('is_active', true)->get();
            foreach ($deals as $deal) {
                $price = (float) ($deal->discount_value ?: $deal->price ?: 0);
                $items[] = [
                    'id' => null,
                    'deal_id' => $deal->id,
        ...
        $dbItems = $query->orderBy('sort_order')->get();
    ...
    private function buildCategoriesMenu(Restaurant $restaurant): string
    {
        $categories = $this->getFlowCategories($restaurant);
        if (empty($categories)) {
        ...
        $msg = "Assalam-o-Alaikum! Welcome to *{$restaurant->name}* 🍽️\n\n";
        $msg .= "📋 *Menu Categories:*\n───────────────────\n";
    ...
    private function buildCategoryItemsMenu(Restaurant $restaurant, $categoryId): string
    {
        $items = $this->getFlowItemsForCategory($restaurant, $categoryId);
        if (empty($items)) {
        ...
        foreach ($items as $idx => $item) {
            $num = $idx + 1;
            $msg .= "{$num}. *{$item['name']}* — Rs." . number_format($item['price'], 0) . "\n";
    ...
    private function parseFlowItemSelections(string $text, array $items): array
    {
        $results = [];
        if (empty($text) || empty($items)) return $results;

        $segments = array_filter(array_map('trim', preg_split('/[,;\n]+/', $text)));

        foreach ($segments as $seg) {
            if (preg_match('/^(\d+)\s*[*xX×]\s*(\d+)$/u', $seg, $m)) {
                $v1 = (int) $m[1];
                $v2 = (int) $m[2];
                if ($v1 >= 1 && $v1 <= count($items)) {
                    $results[] = ['item' => $items[$v1 - 1], 'quantity' => $v2];
                    continue;
                }
                if ($v2 >= 1 && $v2 <= count($items)) {
                    $results[] = ['item' => $items[$v2 - 1], 'quantity' => $v1];
                    continue;
                }
            }
            if (preg_match('/^(\d+)$/', $seg, $m)) {
                ...
                if ($idx >= 1 && $idx <= count($items)) {
                    $results[] = ['item' => $items[$idx - 1], 'quantity' => 1];
                    continue;
                }
            }
        }
    ...
    private function addFlowCartItem(array &$cart, array $item, int $quantity = 1): void
    {
        $qty = max(1, min($quantity, 50));
        foreach ($cart as &$ci) {
            if ($ci['name'] === $item['name'] && ($ci['menu_item_id'] ?? null) === ($item['id'] ?? null)) {
                $ci['quantity'] += $qty;
    ...
    private function buildCartSummary(array $cart, float $deliveryCharge): string
    {
        if (empty($cart)) {
        ...
        $subtotal = 0.0;
        $lines = "🛒 *Aapka Shopping Cart:*\n───────────────────\n";
        foreach ($cart as $idx => $item) {
            $num = $idx + 1;
            $sub = $item['unit_price'] * $item['quantity'];
            $subtotal += $sub;
            $lines .= "{$num}. *{$item['quantity']}x* {$item['name']} — Rs." . number_format($sub, 0) . "\n";
        }
        $total = $subtotal + $deliveryCharge;
        $lines .= "───────────────────\n";
        $lines .= "Subtotal: Rs." . number_format($subtotal, 0) . "\n";
        $lines .= "Delivery Charge: Rs." . number_format($deliveryCharge, 0) . "\n";
        $lines .= "*Total Payable: Rs." . number_format($total, 0) . "*\n───────────────────\n\n";
    ...
    private function buildOrderSummaryFromCart(array $cart, array $orderData, float $deliveryCharge): string
    {
        $subtotal = 0.0;
        $lines = "🧾 *Order Summary*\n───────────────────\n";
        foreach ($cart as $item) {
            $sub = $item['unit_price'] * $item['quantity'];
            $subtotal += $sub;
            $lines .= "{$item['quantity']}x {$item['name']} — Rs." . number_format($sub, 0) . "\n";
        }
        $total = $subtotal + $deliveryCharge;
        $lines .= "───────────────────\n";
        $lines .= "Subtotal: Rs." . number_format($subtotal, 0) . "\n";
        $lines .= "Delivery: Rs." . number_format($deliveryCharge, 0) . "\n";
        $lines .= "*Total: Rs." . number_format($total, 0) . "*\n";
        $lines .= "───────────────────\n";
        $lines .= "👤 Name: " . ($orderData['customer_name'] ?? 'Customer') . "\n";
        $lines .= "📱 Phone: " . ($orderData['contact_phone'] ?? 'WhatsApp Number') . "\n";
        $payMethod = ($orderData['payment_method'] ?? '') === 'jazzcash'
            ? 'JazzCash 📱'
            : (($orderData['payment_method'] ?? '') === 'easypaisa' ? 'EasyPaisa 📱' : 'Cash on Delivery (COD) 💵');
        $lines .= "💵 Payment: {$payMethod}\n";
        $lines .= "📍 Deliver to: " . ($orderData['delivery_address'] ?? 'Address not specified') . "\n\n";
        $lines .= "Kya main aapka order confirm kar doon? ✅\n\n";
        $lines .= "👉 Reply karein *'YES'* ya *'CONFIRM'* order place karne ke liye!\n";
        if (($orderData['payment_method'] ?? 'cash_on_delivery') === 'cash_on_delivery') {
            $lines .= "👉 Agar JazzCash se payment karni hai toh *'JAZZCASH'* likhein.";
        }
