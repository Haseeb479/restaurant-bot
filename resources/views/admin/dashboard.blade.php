...
<div class="body">
    @if(session('success'))
        <div class="success-bar"><div class="s-bar-dot"></div>{{ session('success') }}</div>
    @endif
    ...
    <div class="stats">
        ...
        <div class="stat">
            <div class="stat-num">Rs {{ number_format($totalRevenue) }}</div>
            <div class="stat-label">Revenue today</div>
        </div>
        <div class="stat">
            <div class="stat-num">{{ $restaurants->count() }}</div>
    ...
    <div class="table">
        ...
        <div class="t-row">
            <div>
                ...
                <div class="r-meta">{{ $r->orders_count }} orders · {{ $r->menu_items_count }} items
                    @if($r->plan_expires_at) · Expires {{ $r->plan_expires_at->format('M j, Y') }} @endif
                </div>
            </div>
            <div>
                <span class="plan-pill {{ $r->plan === 'pro' ? 'plan-pro' : 'plan-trial' }}">
                    {{ ucfirst($r->plan) }}
                </span>
            </div>
            <div>
                <span class="status-pill {{ $r->is_active ? 's-active' : 's-inactive' }}">
                    <span class="s-dot"></span>{{ $r->is_active ? 'Active' : 'Inactive' }}
                </span>
            </div>
            <div class="action-row">
                <a href="/dashboard/{{ $r->id }}/login" class="view-btn" target="_blank">View</a>
